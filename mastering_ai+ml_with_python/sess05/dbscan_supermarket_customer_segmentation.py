...
# Visit Frequency ( visits/ year )
# Average basket value in Kes. Customer Tenure in months
# Loyalty Score ( 0 - 100 )

# ------------------------------------------------------------------------------------------------------
# 0. Import the requires modules
# ------------------------------------------------------------------------------------------------------
...
def generate_customers(n_customers: int = 1000, seed: int = 42) -> pd.DataFrame:
    """
    Generate a synthetic dataset of customers with embedded behaviour patterns.
    ...
    return (
        pd.DataFrame(records).sample(frac=1, random_state=seed).reset_index(drop=True)
    )
...
def preprocess(df: pd.DataFrame):
    """
    Standardise the features ( critical for DBSCAN distance calculation)
    """
...
def assign_business_segments(df: pd.DataFrame, labels: np.ndarray) -> pd.DataFrame:
    ...
    for cluster in set(labels):
        ...
        elif spend > 50 and diversity > 7 and tenure < 10:
            name = "New Explorers"
        else:
